# EUDI Wallet Implementation Conformance Statement (ICS)

This is the general Implementation Conformance Statement (ICS) form for EUDI Wallet
...
The ICS is used in conformance assessment as follows:
...
- Each test case states its capability/profile conditions in `## Profile applicability`.
  ...
  case is selected when the declared capabilities satisfy those conditions. A test case
...
- The EUDI-wallet column states the constraint that the European Digital Identity
  ...
  standards they make binding) imposes on the declared value, together with the source of the
  constraint. Options struck through are prohibited for EUDI-Wallets; `Yes/~~No~~` marks
  capabilities that every EUDI Wallet must declare.
...
This list is non-exhaustive. It covers only items for which test coverage has been defined in the FCAF.

Items with the ID `N/A` are surrogate ICS items. They are mandatory for EUDI Wallets and can be used to select relevant test cases through the Technology section.

## Generic

| ID | ICS item | Options | EUDI-wallet specifics |
| --- | --- | --- | --- |
| N/A | Wallet supports credentials in IETF SD-JWT VC format. | Yes/~~No~~ | EUDI\_required; [CIR 2024/2979 amended] Article 8 with Annex II ([ETSI TS 119 472-1] clause 5): wallet solutions shall support the usage of PID and EAAs issued in compliance with the listed standards, which include the SD-JWT VC format. |
| N/A | Wallet supports credentials in ISO/IEC 18013-5 mdoc format. | Yes/~~No~~ | EUDI\_required; [CIR 2024/2979 amended] Article 8 with Annex II ([ETSI TS 119 472-1] clause 6, ISO/IEC-mdoc format); for presentation additionally [CIR 2024/2982 amended] Annex II ([ETSI TS 119 472-2] clause 5.2, ISO/IEC 18013-SUPPORT-02: the EUDI Wallet shall meet the ISO/IEC-mdoc profile protocol requirements). |
...
## Presentation-related capabilities

| ID | ICS item | Options | EUDI-wallet specifics |
| --- | --- | --- | --- |
| N/A | Wallet supports invocation through the custom URL scheme `eu-eaap://`. | Yes/~~No~~ | EUDI\_required; [ETSI TS 119 472-2] clause 6.4.1 (OIDFVP-HAIP-REDIRECTS-03), IA-bound via [CIR 2024/2982 amended] Annex II. |
| P.1 | Wallet supports invocation through the custom URL scheme `haip-vp://`. | Yes/No | |
| N/A | Wallet supports remote presentations using OpenID4VP (as profiled by HAIP and ETSI TS 119 472-2). | Yes/~~No~~ | EUDI\_required; [ETSI TS 119 472-2] clause 6.2 (OIDFVP-HAIP-SUPPORT-01), IA-bound via [CIR 2024/2982 amended] Annex II. |
| N/A | Wallet supports presentations via redirects (non-API mediated). | Yes/~~No~~ | EUDI\_required; [ETSI TS 119 472-2] clauses 6.2 and 6.4, IA-bound via [CIR 2024/2982 amended] Annex II. |
...
| N/A | Wallet supports cross-device presentation flows. | Yes/No | EUDI\_optional; [CIR 2024/2982 amended] Annex II, clause 6.2 (OIDFVP-HAIP-SUPPORT-03) says the Wallet should not support the redirects-based mechanism for cross-device flows, but its Note 3 states that implementing this mechanism should not trigger non-compliance. |
| N/A | Wallet supports presentations via the W3C Digital Credentials API (or equivalent platform API). | Yes/~~No~~ | EUDI\_required; [CIR 2024/2982 amended] Annex II, clause 4.3 (EAAP-API-GEN-01), requires a mediating API supporting both the HAIP 5.2 protocol and ISO/IEC 18013-7 Annex C. Failures caused by missing OS/browser support are non-conformities attributed to the platform (NOTE to EAAP-API-GEN-01). |
| P.2 | Wallet supports the Request URI Method `post`. | Yes/No | TBD |
| N/A | Wallet supports `response_mode=direct_post.jwt` (encrypted authorization responses). | Yes/~~No~~ | EUDI\_required for redirects; [HAIP] clause 5.1 requires `direct_post.jwt`, applied through [ETSI TS 119 472-2] clause 6.4.1 (OIDFVP-HAIP-REDIRECTS-01) and [CIR 2024/2982 amended] Annex II. |

## Issuance-related capabilities

| ID | ICS item | Options | EUDI-wallet specifics |
| --- | --- | --- | --- |
| I.1 | Wallet supports invocation through the custom URL scheme `haip-vci://`. | Yes/No | |
...
| N/A | Wallet supports Credential Issuance using the Authorization Code Flow. | Yes/~~No~~ | EUDI\_required; [CIR 2024/2982 amended] Annex I applies [ETSI TS 119 472-3] clause 4.1 (GEN-REQ-4.1-03). |
| N/A | Wallet supports Credential Issuance using the Pre-Authorization Code Flow. | Yes/~~No~~ | EUDI\_required; [CIR 2024/2982 amended] Annex I applies [ETSI TS 119 472-3] clause 4.1 (GEN-REQ-4.1-03). |

...
## Objective
Verify that the Wallet does not present a credential to a Relying Party, if a Wallet Relying Party Registration Certificate with an invalid signature is used, when using OpenID4VP presentation.
...
B. Wallet trusts the CA issuing a Relying Party's WRPAC (i.e. through the EUDI trust lists).
C. Wallet trusts the CA issuing a Relying Party's WRPRC (i.e. through the EUDI trust lists).
D. Wallet and Verifier are engaged, and a presentation using OpenID4VP has been triggered.
...
1. Verifier sends a Request Object to the Wallet. The Request Object:
    ...
    4. contains a `verifier_info` property, with the following properties:
        ...
        2. has a `data` property with as value a string with the base64url encoding of a WRPRC, where
            1. the WRPRC has an invalid signature value (e.g. using a XOR pattern on a valid signature value).
            2. the WRPRC's `sub` field has the same value as the WRPAC's `organizationIdentifier`.
            3. the WRPRC does not contain a `usesIntermediary` field.

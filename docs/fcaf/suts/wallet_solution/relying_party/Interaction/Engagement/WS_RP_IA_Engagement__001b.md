# WS_RP_IA_Engagement_001b

## Objective
Verify that, in a presentation flow via Redirects, the Wallet can be invoked through the custom URL scheme `eu-eaap://`.

## References
[ETSI TS 119 472-2] section 6.4.1 
[HAIP] section 5.1
[OpenID4VP] section 9

## EUDI-wallet relevancy
EUDI_specific | EUDI_required

## Profile applicability
None

## Technology
Presentation via Redirects
Wallet supports to be invoked through the custom URL scheme "eu-eaap://".

## Preconditions
A. The End-user is engaging with a Verifier using a User-agent.

## Test Scenario
1. Verifier provides a presentation link to the User-agent using the custom URL scheme `eu-eaap://`.
    NOTE: The EUDI Wallet should not support this redirects-based mechanism for cross-device presentation flows, e.g. by presenting the link as QR-code; implementing it for cross-device use should not by itself trigger non-compliance.
2. End-user triggers the link to be followed (e.g. clicks a button or link).

## Expected results
1. User-agent presents option to End-user to engage presentation.
2. Wallet is invoked successfully.


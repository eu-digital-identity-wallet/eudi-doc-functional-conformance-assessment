# WS_RP_IA_ProtocolFlow_002c

## Objective
Verify that, in the presentation flow via Redirects, the Wallet supports receiving a Signed Authorization Request using JWT-Secured Authorization Request (JAR, [RFC9101]) through the `request_uri` parameter, when the `request_uri_method` parameter is omitted.
...
1. The Verifier provides an invocation URL for a OID4VP request via redirects using Request Object by reference (i.e. `eu-eaap://` link), with
    ...
    2. omitting the `request_uri_method` parameter.
...
4. Wallet responds with a presentation of 'default_credential_A'.

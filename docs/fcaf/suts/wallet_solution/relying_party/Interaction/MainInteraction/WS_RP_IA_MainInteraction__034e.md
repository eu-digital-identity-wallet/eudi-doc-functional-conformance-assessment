# WS_RP_IA_MainInteraction_034e

## Objective
Verify that the Wallet responds to a DCQL-query selecting all elements of a top-level array claim with a `null` path component, and does not include other selectively disclosable claims, for a credential in SD-JWT VC format.
...
1. Verifier sends a request with a valid DCQL query, with a `credentials` property that is an array containing one object with the following properties:
    ...
    2. the `claims` property, as an array with one object containing a `path` property with as value an array containing two elements,
        1. the first a string, with the name of a top-level claim that is an array of which all elements are selectively disclosable in 'default_credential_A'.
        2. the second a `null` value.
...
2. The presentation has the following characteristics:
    ...
    2. The presentation contains the disclosures of all elements of the selected top-level array claim.

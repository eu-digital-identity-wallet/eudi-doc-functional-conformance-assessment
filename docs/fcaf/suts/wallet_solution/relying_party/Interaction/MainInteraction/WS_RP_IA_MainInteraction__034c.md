# WS_RP_IA_MainInteraction_034c

## Objective

Verify that the Wallet responds to a DCQL-query selecting one element of a top-level array claim by its array index, and does not include other selectively disclosable claims, for a credential in SD-JWT VC format.
...
1. Verifier sends a request with a valid DCQL query, with a `credentials` property that is an array containing one object with the following properties:
    ...
    2. the `claims` property, as an array with one object containing a `path` property with as value an array containing two elements,
        ...
        2. the second an integer (e.g. 1) that is an index present in that array.
...
2. The presentation has the following characteristics:
    ...
    2. The presentation contains the disclosure of the array element selected by the requested index.

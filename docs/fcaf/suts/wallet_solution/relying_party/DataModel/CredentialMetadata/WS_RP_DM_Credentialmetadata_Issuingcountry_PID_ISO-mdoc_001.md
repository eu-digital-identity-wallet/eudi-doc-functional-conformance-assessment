# EuPid_DM_IssuingCountry_01

## Objective
This test case verifies that the data element issuing_country is present in the mdoc data.
...
The EUDI wallet contains a document with DocType = "eu.europa.ec.eudi.pid.1". Data element issuing_country is present in the mdoc data.
...
## Test Scenario
1. Verify the presence of a data element with identifier issuing_country in the device retrieval mdoc response. 2. Verify the presence of an ErrorItem for data element issuing_country.

## Expected results
1. One data element with identifier issuing_country is present. 2. There is no ErrorItem for data element issuing_country.
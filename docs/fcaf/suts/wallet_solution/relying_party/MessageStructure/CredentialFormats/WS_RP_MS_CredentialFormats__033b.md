# WS_RP_MS_CredentialFormats_033b

## Objective

Verify that the Wallet includes a correctly formatted `status` claim in a mdoc presentation, when the issuer included the status information in the credential.
...
5. The MSO of the presented mdoc contains a `status` key.
...
1. The value of the `status` element in the MSO is a CBOR Map (major type 5).

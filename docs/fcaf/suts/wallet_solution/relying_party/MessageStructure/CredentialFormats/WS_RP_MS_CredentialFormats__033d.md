# WS_RP_MS_CredentialFormats_033d

## Objective
Verify that the Wallet includes a correctly formatted `status_list` element in the `status` element in a mdoc presentation, when the issuer included the status information in the credential in Token Status List format.
...
F. The `status` element in the MSO of the presented mdoc contains a `status_list` key.

## Test Scenario
1. Verify the `status_list` element in the `status` element in the MSO of the presented mdoc credential.

## Expected results
1. The value of the `status_list` element in the `status` element in the MSO is a CBOR Map (major type 5).

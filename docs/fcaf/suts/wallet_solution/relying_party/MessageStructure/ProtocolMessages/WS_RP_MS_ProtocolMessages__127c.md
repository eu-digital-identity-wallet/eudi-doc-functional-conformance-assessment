...
B. Verifier requested credential(s) to be presented, using a valid, trusted request for credential(s) in SD-JWT VC format.
C. The Authorization Response from the Wallet is received by the Verifier.
D. The Authorization Response is successfully decrypted by the Verifier.
E. The Authorization Response contains a syntactically correct `vp_token` parameter.
F. The `vp_token` contains syntactically correct presentation(s).

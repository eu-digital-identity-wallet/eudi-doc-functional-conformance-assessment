...
E. The HTTP Request to the PAR Endpoint of the Authorization Server contains a syntactically correct `OAuth-Client-Attestation` HTTP header.
F. The Client Attestation used by the Wallet to authenticate to the PAR Endpoint is a correctly serialized signed JWT.
G. The Client Attestation uses an acceptable signature algorithm.
